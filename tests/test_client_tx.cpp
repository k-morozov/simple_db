...
TEST(TestClientTx, SimplePut) {
  ...
  client::TxSpec spec{1,  // start timestamp
                      4,  // commit timestamp
                      {{
                          // Get
                          2,  // start timestamp
...
TEST(TestClientTx, DISABLED_SimpleConflictedPuts) {
  const sdb::tx::ActorID node_id = 1;
  const sdb::tx::ActorID client = 10;

  client::TxSpec spec1{1,  // start timestamp
                       4,  // commit timestamp
                       {},
                       {{
                           // Put
  ...
  client::TxSpec spec2{1,  // start timestamp
                       3,  // commit timestamp
                       {},
                       {{
                           // Put
  ...
  {  // send put reply for tx1
    in1.push_back(
        msg::CreateMsgPutReply(node_id, client, txid1, out1[0].msg_id));
    out1.clear();
    tx1.tick(7, in1, &out1);
  ...
  {  // send put reply for tx2
    in2.push_back(
        msg::CreateMsgPutReply(node_id, client, txid2, out2[0].msg_id));
    out2.clear();
    tx2.tick(8, in2, &out2);
