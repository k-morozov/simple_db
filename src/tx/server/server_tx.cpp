...
ServerTX::ServerTX(ActorID clientID, Storage* storage, Retrier* retrier)
    : clientID_(clientID), storage_(storage), retrier_(retrier) {
  assert(storage_);
  assert(retrier_);
...
void ServerTX::tick(const Timestamp ts, Messages msgs,
...
void ServerTX::process_msg_not_started(const Timestamp ts,
  ...
  switch (msg.type) {
    case msg::MessageType::MSG_START: {
      ...
      const auto ack_msg = msg::CreateMsgStartAck(
          source_for_ack, destination_for_ack, txid_, read_ts_);
      LOG_DEBUG << "[ServerTX::process_msg_not_started] schedule ack: "
                << ack_msg;

      [[maybe_unused]] auto handle = retrier_->schedule(read_ts_, ack_msg);

      LOG_INFO << "[ServerTX::process_msg_not_started] change state from "
               << to_string(state_) << " to " << to_string(ServerTXState::OPEN);
...
void ServerTX::process_msg_open(Timestamp ts, msg::Message msg) {
  ...
  switch (msg.type) {
    case msg::MessageType::MSG_PUT: {
      ...
      auto msg_put_reply = msg::CreateMsgPutReply(msg.destination, msg.source,
                                                  txid_, msg.msg_id);

      LOG_DEBUG << "[ServerTX::process_msg_open] send put reply: "
                << msg_put_reply;
    ...
    case msg::MessageType::MSG_COMMIT: {
      commit_ts_ = ts;
      LOG_DEBUG << "[ServerTX::process_msg_open][txid=" << txid_
                << "][read_ts=" << read_ts_
                << "] setup commit_ts=" << commit_ts_;

      bool has_conflict = false;
      TxID conflict_txid{UNDEFINED_TX_ID};

      LOG_DEBUG << "[ServerTX::process_msg_open][txid=" << txid_ << "] has " << puts_.size() << " puts.";
      for (const auto& put : puts_) {
        LOG_DEBUG << "[ServerTX::process_msg_open][txid=" << txid_ << "] put=" << put ;
        const auto* p = storage_->get_last(put.key);
        if (!p) {
            LOG_DEBUG << "[ServerTX::process_msg_open][txid=" << txid_ << "] no key in storage." ;
            continue;
        }

        const Timestamp old_key_ts = p->first;
        LOG_DEBUG << "[ServerTX::process_msg_open] in storage key=" << put.key
                  << " has last: value=" << p->second
                  << " and ts=" << old_key_ts;

        if (old_key_ts > read_ts_) {
            LOG_DEBUG << "[ServerTX::process_msg_open][txid=" << txid_ << "] found conflict"
                << "old_key_ts=" << old_key_ts << ", read_ts=" << read_ts_;
            has_conflict = true;
      ...
      if (has_conflict) {
        ...
        const auto msg_rolled_back = msg::CreateMsgRolledBackByServer(
            msg.destination, msg.source, txid_, conflict_txid);
      ...
      for (const auto& put : puts_) {
        LOG_DEBUG << "[ServerTX::process_msg_open][txid=" << txid_ << "] apply put " << put << " to storage.";
        storage_->apply(txid_, commit_ts_, put.key, put.value);
      }

      const auto msg_commit_ack = msg::CreateMsgCommitAck(
          msg.destination, msg.source, txid_, commit_ts_);
...
void ServerTX::report_unexpected_msg(const msg::Message msg) {
  LOG_ERROR << "[ServerTX][" << txid_ << "]"
            << " state=" << to_string(state_) << " gon unexpected msg=" << msg;
}

std::ostream &operator<<(std::ostream &os, const ServerTX::DataPut &put) {
    os << "DataPut: key=" << put.key << ", value=" << put.value;
    return os;
}
